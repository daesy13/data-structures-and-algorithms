...
const hasChildrenValues = (arr, character) => {
  let result;
  const dataUser = Object.values(arr);
...
const hasChildrenEntries = (arr, character) => {
  let result;
  const dataUser = Object.entries(arr);
  dataUser.forEach(elem => {
    if (elem[1].name === character){
      if (elem[1].children.length > 0){
        result = true;
      }
      else {
        result = false;
...
const totalCharacters = (arr) => {
  let result = 0;
  for (let things of arr){
    if (Object.values(things)[0] !== null){
      result++;
    }
    if (Object.values(things)[1] !== null){
      result++;
    }
    if (Object.values(things)[2].length > 0){
      result += Object.values(things)[2].length;
    }
  }
  return result;
};
